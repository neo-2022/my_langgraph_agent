...
1. [ ] Создать `docs/integration/REGART_ART_CONTRACT.md`  
...
2. [ ] Зафиксировать политику **unknown fields** (обратная совместимость)  
...
3. [ ] Зафиксировать **partial-ack формат** (обязательный пример JSON)  
...
4. [ ] Зафиксировать **Retry/Timeout policy** (конкретные числа)  
...
5. [ ] UI: добавить RawEvent schema + normalize  
   **Технология:** zod  
   **Артефакты:**  
...
6. [ ] UI Proxy: pydantic модели RawEvent (**extra=ignore**)  
...
7. [ ] Добавить `schema_version` в событие + upgrade-конвертеры  
   **Артефакты:**  
   - `ui/src/obs/rawEvent.upgrade.js`  
   - `agent/src/react_agent/obs_upgrade.py`  
   **Проверка:** файлы существуют.

8. [ ] Добавить тест “**v2 событие валидируется схемой v1** (unknown fields ignored)”  
   **Проверка:** unit-тесты зелёные (см. шаги 57–59).

9. [ ] Добавить тест “**rolling upgrade**: старая версия → пишет в spool/outbox → новая версия доставляет”  
...
10. [ ] Реализовать `client_id` server-side (UI Proxy) + `session_id` в UI  
    **Проверка:** UI получает `client_id` как non-secret; секретов нет в UI.

11. [ ] Защитить ingress: `/ui/ingest/events` и `/ui/ingest/attachments` (bearer token + allowlist)  
    **Проверка:** pytest: без токена 401/403, с токеном 200 (см. шаг 60).

12. [ ] Гарантировать: REGART→Art авторизация добавляется **только** в UI Proxy  
    **Проверка:**
    ~~~bash
...
13. [ ] UI: генерировать `trace_id/span_id/request_id` и прокидывать заголовками  
    **Проверка:** unit-тест + grep (см. шаг 58).

14. [ ] UI Proxy: прокидывать корреляцию в upstream (LangGraph/Providers)  
    **Проверка:** интеграционный тест проверяет заголовки на стороне mock upstream (см. шаг 61).

---

## 4) Единый сетевой слой UI + запрет прямого fetch

15. [ ] Создать `ui/src/obs/httpClient.js` и перевести UI на него  
    **Проверка:**
    ~~~bash
    rg -n "fetch\\(" ui/src | rg -v "obs/httpClient\\.js|node_modules" || true
    ~~~

16. [ ] Инструментировать сеть `network.request/response/error` (RawEvent)  
    **Проверка:** unit + e2e (см. шаги 58–59).

17. [ ] Ввести **таймауты** на UI-вызовы (AbortController)  
...
18. [ ] Level0 (до React) пишет RawEvent и кладёт в outbox  
    **Проверка:** e2e “ошибка до React” (см. шаг 59).

19. [ ] UiErrorCore выдаёт только RawEvent (или строгий маппинг)  
...
20. [ ] Реализовать outbox в IndexedDB  
    **Технология:** Dexie  
    **Артефакты:** `ui/src/obs/outbox.db.js`, `ui/src/obs/outbox.js`  
    **Проверка:** перезагрузка не теряет очередь.

21. [ ] Оптимизация outbox: **lazy/paged load**, лимиты (max_count/max_bytes/max_age), батч-лимит  
    **Проверка:** unit-тест: загрузка страницы не читает всю базу; батч ограничен.

22. [ ] **Ordering (если важен):** хранить `sequence_id` (монотонный per session_id)  
    **Проверка:** unit-тест: sequence возрастает и отправка идёт по нему.

23. [ ] Если ordering важен: отправка **строго последовательно** для одной session (без параллельных flush)  
    **Проверка:** unit-тест “двойной flush” не запускает 2 отправки.

24. [ ] Реализовать обработку **partial-ack** (per-event statuses)  
    - accepted → ack & delete  
    - retryable → остаётся  
    - rejected permanent → DLQ  
    **Проверка:** unit-тест на разбор ответа + действия.

25. [ ] Реализовать **DLQ в UI** + событие `observability_gap.dlq_enqueued`  
...
27. [ ] Усилить уникальность `event_id` + (опц.) `content_hash`  
...
28. [ ] UI отправляет батчи в UI Proxy (`/ui/art/ingest`) без секретов  
    **Проверка:** network события показывают запросы только на UI Proxy.

29. [ ] UI Proxy форвардит в Art с **timeout policy** (connect/read/write)  
    **Проверка:** тест: Art “завис” → UI Proxy завершает по таймауту, событие retryable (см. шаг 60).

30. [ ] UI Proxy поддерживает **partial-ack** (не переотправлять успешные)  
    **Проверка:** интеграционный тест на mixed statuses.

31. [ ] Server-spool (SQLite) для оффлайна Art  
    **Технология:** SQLite + aiosqlite, WAL  
    **Проверка:** Art off → события пишутся в sqlite.

32. [ ] Оптимизация SQLite: индексы + очистка acked + политика retention  
...
33. [ ] UI Proxy **dedup** по `event_id` (и `source`) до отправки в Art  
...
34. [ ] UI Proxy DLQ (server-side) + алертинг как в шаге 26  
    **Проверка:** событие попадает в dlq таблицу, метрика растёт, gap-событие генерируется.

35. [ ] Graceful degradation при повреждении SQLite spool  
...
36. [ ] Миграции схемы SQLite spool  
...
37. [ ] `/ui/ingest/events`: auth + pydantic validation + redaction  
    **Проверка:** pytest (см. шаг 60).
