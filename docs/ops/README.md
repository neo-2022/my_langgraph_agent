...
| `ART_INGEST_URL` | UI Proxy → Art ingest | `https://127.0.0.1:7331/api/v1/ingest` | URL, куда UI Proxy форвардит события. По умолчанию требуется `https://`; `http://` допускается только для localhost при `ART_ALLOW_HTTP_LOCAL=1`. |
| `ART_STREAM_URL` | UI Proxy → Art stream | `https://127.0.0.1:7331/api/v1/stream` | SSE-канал, к которому проксируется UI stream. По умолчанию требуется `https://`; `http://` допускается только для localhost при `ART_ALLOW_HTTP_LOCAL=1`. |
| `ART_ACTIONS_URL` | UI Proxy → Art actions | `https://127.0.0.1:7331/api/v1/actions/execute` | Управление сервисами только через Art Actions API, прямые системные команды для управления запрещены. Для `http://` действуют те же ограничения localhost + `ART_ALLOW_HTTP_LOCAL=1`. |
| `ART_TLS_VERIFY` | TLS валидация cert chain | `1` | `1/true` — строгая валидация сертификата; `0/false` — dev-режим для self-signed сертификатов. |
| `ART_ALLOW_HTTP_LOCAL` | локальный HTTP fallback для Art URL | `1` | Разрешает `http://127.0.0.1`, `http://localhost`, `http://[::1]` для dev-окружения. Для остальных хостов остаётся обязательный `https://`. |
